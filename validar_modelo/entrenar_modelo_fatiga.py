...
# =============================================================================
# ENTRENAMIENTO DEL MODELO - PREDICCION DE FATIGA DE JUGADORES
# =============================================================================
# Modelo: XGBoost Regressor
# Input: Variables fisicas, de carga y contexto del jugador
# Output: Nivel de fatiga (0-100)
# =============================================================================


def cargar_y_preparar_datos(csv_path='ftr_fatiga_medica.csv'):
    print("[1/6] Cargando dataset...")
    df = pd.read_csv(csv_path)
    print(f"       Dataset cargado: {df.shape[0]} filas, {df.shape[1]} columnas")

    columnas_excluir = ['fatiga']
    ...
    print(f"       Target: 'fatiga' (min={y.min():.1f}, max={y.max():.1f}, media={y.mean():.1f})")
...
def evaluar_modelo(modelo, X_train, y_train, X_test, y_test):
    ...
    print(f"   MAE (fatiga):  {mae_train:>10.2f}  {mae_test:>10.2f}")
    print(f"   RMSE (fatiga): {rmse_train:>10.2f}  {rmse_test:>10.2f}")
    ...
    # Error por rango de fatiga
    print(f"\n[ERROR POR RANGO DE FATIGA]")
    rangos = [(5, 25), (25, 45), (45, 65), (65, 85), (85, 96)]
    for low, high in rangos:
        mask = (y_test >= low) & (y_test < high)
        if mask.sum() > 0:
            mae_r = mean_absolute_error(y_test[mask], y_pred_test[mask])
            print(f"   {low:>2}-{high:>2} fatiga: MAE={mae_r:.2f} ({mask.sum()} registros)")
    ...
    for idx in indices:
        ...
        diff = abs(real - pred)
        print(f"   Real: {real:>5.1f} | Predicho: {pred:>5.1f} | Error: {diff:>4.1f}")

    return r2_test, mae_test


def guardar_modelo(modelo, encoders, columnas_features, ruta='modelo_fatiga_entrenado'):
...
def validacion_cruzada(X, y):
    ...
    print(f"       MAE Media: {-scores_mae.mean():.2f} puntos fatiga (+/- {scores_mae.std():.2f})")

    if scores_r2.std() < 0.03:
        print("       [OK] Modelo estable")
    else:
        print("       [!] Alta varianza entre folds")
...
if __name__ == '__main__':
    ...
    X, y, columnas_features = cargar_y_preparar_datos('ftr_fatiga_medica.csv')
    ...
    guardar_modelo(modelo, encoders, columnas_features, ruta='modelo_fatiga_entrenado')
    ...
    print(f"MAE: {mae:.2f} puntos de fatiga")
    print(f"Modelo guardado en: modelo_fatiga_entrenado/")
    print(f"Para predecir: python predecir_fatiga.py")
